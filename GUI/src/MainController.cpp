...
MainController::MainController(int argc, char *argv[])
  ...
  if (logFile.length()) {
    logReader = new RawLogReader(
        logFile, Parse::get().arg(argc, argv, "-f", empty) > -1);
  } else {
    bool flipColors = Parse::get().arg(argc, argv, "-f", empty) > -1;
    logReader = new LiveLogReader(logFile, flipColors,
                                  LiveLogReader::CameraType::OpenNI2);
    ...
    if (!good) {
      delete logReader;
      logReader = new LiveLogReader(logFile, flipColors,
                                    LiveLogReader::CameraType::RealSense);
  ...
  end = std::numeric_limits< unsigned short >::max();  // Funny bound, since we
                                                       // predict times in this
                                                       // format really!
  ...
  openLoop =
      !groundTruthOdometry && Parse::get().arg(argc, argv, "-o", empty) > -1;
  ...
  gui = new GUI(logFile.length() == 0,
                Parse::get().arg(argc, argv, "-sc", empty) > -1);
  ...
  while (good) {
    ...
    if (eFusion == 0 || resetButton) {
      ...
      eFusion = new ElasticFusion(
          ...
          frameToFrameRGB, logReader->getFile());
    } else {
      break;
    }
  }
...
void MainController::run() {
  while (!pangolin::ShouldQuit() && !((!logReader->hasMore()) && quiet) &&
    ...
    if (!gui->pause->Get() || pangolin::Pushed(*gui->step)) {
      if ((logReader->hasMore() || rewind) && eFusion->getTick() < end) {
        ...
        eFusion->processFrame(logReader->rgb, logReader->depth,
                              logReader->timestamp, currentPose,
                              weightMultiplier);
        ...
        if (frameskip &&
            Stopwatch::getInstance().getTimings().at("Run") > 1000.f / 30.f) {
          framesToSkip = int(Stopwatch::getInstance().getTimings().at("Run") /
                             (1000.f / 30.f));
        }
      }
    } else {
    ...
    stre << (std::isnan(eFusion->getModelToModel().lastICPError)
                 ? 0
                 : eFusion->getModelToModel().lastICPError);
    gui->trackRes->Ref().Set(stre.str());

    if (!gui->pause->Get()) {
      gui->resLog.Log((std::isnan(eFusion->getModelToModel().lastICPError)
    ...
    if (gui->drawRawCloud->Get()) {
      eFusion->getFeedbackBuffers()
          ...
          ->render(gui->s_cam.GetProjectionModelViewMatrix(), pose,
    ...
    if (gui->drawGlobalModel->Get()) {
      ...
      if (gui->drawFxaa->Get()) {
        gui->drawFXAA(gui->s_cam.GetProjectionModelViewMatrix(),
                      ...
                      eFusion->getConfidenceThreshold(), eFusion->getTick(),
                      eFusion->getTimeDelta(), iclnuim);
      } else {
        eFusion->getGlobalModel().renderPointCloud(
            ...
            gui->drawTimes->Get(), eFusion->getTick(), eFusion->getTimeDelta());
      }
      glFinish();
      TOCK("Global");
    }
    ...
    if (gui->drawFerns->Get()) {
      ...
      for (size_t i = 0; i < eFusion->getFerns().frames.size(); i++) {
        if ((int)i == eFusion->getFerns().lastClosest) continue;
    ...
    if (gui->drawDefGraph->Get()) {
      const std::vector< GraphNode * > &graph =
          eFusion->getLocalDeformation().getGraph();
    ...
    if (eFusion->getFerns().lastClosest != -1) {
      glColor3f(1, 0, 0);
      gui->drawFrustum(
          eFusion->getFerns().frames.at(eFusion->getFerns().lastClosest)->pose);
      glColor3f(1, 1, 1);
    }
    ...
    for (std::map< std::string, GPUTexture * >::const_iterator it =
             eFusion->getTextures().begin();
         it != eFusion->getTextures().end(); ++it) {
      if (it->second->draw) {
