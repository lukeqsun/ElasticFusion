...
class GPUConfig {
 ...
 private:
  GPUConfig()
    ...
    icpStepMap["GeForce GTX 770M"] = std::pair<int, int>(128, 80);
    rgbStepMap["GeForce GTX 770M"] = std::pair<int, int>(288, 16);
    rgbResMap["GeForce GTX 770M"] = std::pair<int, int>(256, 80);
    so3StepMap["GeForce GTX 770M"] = std::pair<int, int>(256, 16);
    
    icpStepMap["NVIDIA Tegra X1"] = std::pair< int, int >(416, 16);
    rgbStepMap["NVIDIA Tegra X1"] = std::pair< int, int >(416, 16);
    rgbResMap["NVIDIA Tegra X1"] = std::pair< int, int >(464, 496);
    so3StepMap["NVIDIA Tegra X1"] = std::pair< int, int >(16, 128);

    if (icpStepMap.find(dev) == icpStepMap.end()) {
      std::stringstream strs;
      strs << "Your GPU \"" << dev
           << "\" isn't in the ICP Step performance database, please add it";
    ...
    if (rgbStepMap.find(dev) == rgbStepMap.end()) {
      std::stringstream strs;
      strs << "Your GPU \"" << dev
           << "\" isn't in the RGB Step performance database, please add it";
      std::cout << strs.str() << std::endl;
    } else {
    ...
    if (rgbResMap.find(dev) == rgbResMap.end()) {
      std::stringstream strs;
      strs << "Your GPU \"" << dev
           << "\" isn't in the RGB Res performance database, please add it";
      std::cout << strs.str() << std::endl;
    } else {
    ...
    if (so3StepMap.find(dev) == so3StepMap.end()) {
      std::stringstream strs;
      strs << "Your GPU \"" << dev
           << "\" isn't in the SO3 Step performance database, please add it";
      std::cout << strs.str() << std::endl;
    } else {
  ...
  std::map< std::string, std::pair< int, int > > icpStepMap, rgbStepMap,
      rgbResMap, so3StepMap;
};
